...
```javascript
// Fetch configuration
const config = await fetch('http://config.adunblocker.com/server-tag.json')
...
```javascript JavaScript
const response = await fetch('https://config.adunblocker.com/valid_script_sources.json');
...
```javascript
// Old (deprecated)
const sources = await fetch('https://config.adunblocker.com/valid_script_sources.json')
  .then(response => response.json());
...
<CardGroup cols={2}>
  ...
  >
    Learn how to integrate the JavaScript configuration
  </Card>
  <Card
  ...
  >
    Implement ad blocker detection in WordPress
  </Card>
</CardGroup>
